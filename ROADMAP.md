...
- Fullscreen window rule for games — the mechanism turned out to already be fully implemented upstream (`windowrule=fullscreen,class:...`), just undocumented. Confirmed live via xprop that Steam sets WM_CLASS to `steam_app_<appid>` for any game it launches (native or Proton/Wine), so `windowrule=fullscreen,class:^steam_app_.*$` covers a user's whole Steam library with one rule. Activated as the default in both the example config and the user's own; awaiting a relaunch to confirm the game itself actually goes fullscreen (previously it was just tiling next to Steam with no rule active at all).
