...
- Fixed focus-stealing: launching a game and clicking away to another window would immediately snap focus back to the game (`_NET_ACTIVE_WINDOW` requests were honored unconditionally). First attempt (a 1s cooldown) turned out broken once reproduced live - the real reclaim happened 7+ seconds later, and an unrelated Wine helper window stealing focus in between wiped the tracked state first. Redesigned around tracking the specific window the user themselves deliberately moved away from (persists regardless of timing/intervening churn, cleared only when the user goes back to it themselves). Verified by reproducing the exact live failure sequence with real pointer warps and synthetic EWMH messages.
