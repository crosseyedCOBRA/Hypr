...
- Fixed focus-stealing: launching a game and clicking away to another window would immediately snap focus back to the game (`_NET_ACTIVE_WINDOW` requests were honored unconditionally). Took three attempts to get right: a 1s cooldown broke because the real reclaim happened 7+ seconds later; tracking "the specific window the user moved away from" broke because `eventEnter` fires on every window the pointer passes through en route, not just where it settles, so unrelated pass-through crossings kept overwriting the tracked window. Settled on a simpler question - does the *current* focus belong to the user by deliberate choice at all - that doesn't depend on tracking any window's history. Verified with a full sandbox suite (real pointer warps, synthetic EWMH messages, rapid-cycling stress test) covering the exact live failure sequence plus a redundant-self-reclaim edge case found during testing.
